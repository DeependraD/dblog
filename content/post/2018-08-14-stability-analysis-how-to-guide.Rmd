...
```{r vector-product}
map_dbl(c(1:4), ~ Ri_vec[.x] * cos(theta_mat$V1[.x]*pi/180) * Cj_vec[1])
```
...
wheat_met <- readxl::read_xlsx("./data/35 ESWYT_EnvData copy.xlsx")
wheat_met %>% 
  filter(Country == "NEPAL") %>% 
  head(n=10) %>% 
  knitr::kable(caption = "35th Elite spring wheat yield trial in Nepal") %>% 
  kableExtra::kable_styling(latex_options = c("striped", "scale_down"), position = "center")

wheat_met_raw <- readxl::read_xlsx("./data/35 ESWYT_RawData copy.xlsx")
wheat_met_raw %>% 
  filter(Country == "NEPAL")
```
