...
1. R code is a tree
    ...
    Every expression in R can be broken down to a form represented by a tree.
    For instance, on top of the tree there is "a function call" followed by it's branches: first child = function name itself, other children = function arguments.
    Complex calls have multiple levels of branching.
...
For example, let's create a function that makes scatter plot fed a dataframe, x-variable and y-variable:
...
df.preprocessed1 <- dplyr::as.tbl(params) %>%
...
hareramshyam %>% lapply(length) %>% unlist %>% max -> mymax
hareramshyam %>% lapply(function(x) c(x, rep(NA_integer_, mymax-length(x)))) %>% bind_rows()

## the code above works, but is a two liner
