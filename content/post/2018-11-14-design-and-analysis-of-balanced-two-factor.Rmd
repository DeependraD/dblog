---
title: Design and analysis of balanced two factor randomized complete block (RCBD) experiment
author: Deependra Dhakal
date: '2018-11-14'
slug: design-and-analysis-of-balanced-two-factor
categories:
  - R
  - Plant Breeding
  - agriculture
...
header:
  caption: ''
  image: ''
bibliography: ./bib/2018-11-14-design-and-analysis-of-balanced-two-factor.bib
---
...
The `agricolae::design.rcbd()` function generates design and the field book associated with the design which looks like as shown in Table \@ref(tab:single-factor-des-fieldbook)
...
```{r ihyb-height}
ihyb_multiple <- read_csv("./data/ihyb_2018.csv") %>% 
...
The means for each factor level can be calculated, as well as for each factor level combination.  The notation for the an interaction is "A:B".
...
There are two ways to conduct the variance analysis in R -- one specifying a regression model, other speficying a variance component model. Since, we are dealing with single factor only experiment with replicates, we can use model formula $A+B$ for specifying the regressors or grouping variables (in case of factorial ANOVA), where $A$ is the factor of interest and $B$ is the recogizable source of source variation controlled through replication/blocking. The standard model for a randomized complete block design (with $s = 1$ observation on each treatment in each block) is 
...
```{r ex2data}
ex.2 <- read_csv("./data/data_soybean_sulphur.csv")
head(ex.2)
