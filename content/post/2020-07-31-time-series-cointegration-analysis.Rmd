...
Although not a new topic in finance and portfolio management, as some of the tweets below suggest, it's application in field of Agricultural Economics and commodity market analysis is not often highlighted. A tweet relating S2F and bitcoin (BTC) trading prices is here for reference.

{{< tweet 1235281599454998528 >}}
...
```{r}
# adapted from: https://quant.stackexchange.com/a/3932/48364
set.seed(1001)
bivariate_cointegrated_ts <- function(x0, y0, n = 100L, gamma_v = 0.4, al1, al2, start_date=lubridate::ymd("2010-01-01")) {
  x <- vector(length = n)
  y <- vector(length = n)
  varepsilon1 = rnorm(n, 0, 1)
  varepsilon2 = rnorm(n, 0, 1)
  for (t in 1:99){
    x[1] <- x0
    x[t+1] <- x[t] - gamma_v *(x[t] + (al2/al1)*y[t]) + varepsilon1[t]
    # notice that last value of error will not be used
    y[1] <- y0
    y[t+1] <- y[t] - gamma_v *(y[t] + (al1/al2)*x[t]) + varepsilon2[t]
  }
  return(list(date = seq.Date(from = start_date, 
                              by = 1, 
                              length.out = n), 
              x=x, y=y))
}

coint_ts <- bivariate_cointegrated_ts(x0 = 3.6, y0 = 4.1, al1 = 1, al2 = 1) %>% 
  bind_cols()

ggplot(coint_ts, aes(x = date)) +
  geom_line(aes(y = x)) +
  geom_line(aes(y = y))
```
...
cointegrated.vector(100,3,1,diag(3), burn = 1)
