...
ar1_twenty_sim_ten_phi <- map_dfr(1:20, function(y){map_dfc(set_names(seq(0.1, 0.9, by = 0.1), 
    ...
    pivot_longer(cols = `0.1`:`0.9`, names_to = "phi", values_to = "value")}, .id = "simulation") %>% 
  group_by(phi, simulation) %>% 
  mutate(index = row_number()) %>% 
  pivot_wider(id_cols = c(index, phi, simulation), names_from = simulation, values_from = value)
