...
corr_mat <- read_csv("./../post/data/correlation_matrix.csv")
...
corr_mat <- corr_mat %>% 
  mutate_at(-1, funs(str_remove_all(., "\\*") %>% as.numeric()))
```
...
corr_mat_raw <- corr_mat_raw %>%
  magrittr::set_colnames(corr_mat$X1) %>%
  magrittr::set_rownames(corr_mat$X1)
