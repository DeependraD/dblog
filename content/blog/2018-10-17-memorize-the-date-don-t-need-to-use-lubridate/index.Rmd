...
Suppose you have today's date and current time: Wednesday, Oct 17, 2018, 9:00 AM. Let's start with formatting this character to represent date time using `strptime()`. BTW, you can call `date()` to display current datetime, and as of now in my system, it prints `r date()`.

```{r format-date-time}
today <- c("Wednesday, Oct 17, 2018")
current <- c("Wed, October 17, 2018, 9:00 AM")

sapply(c(today, current), class)

strptime(today, format = "%A, %b %d, %Y", tz = "Asia/Kathmandu") #%A for fullname of weekday, %b for shortname of month

# both are correct
strptime(current, format = "%a, %B %d, %Y, %H:%M", tz = "Asia/Kathmandu") #%H for 24 hour clock
strptime(current, format = "%a, %B %d, %Y, %I:%M %p", tz = "Asia/Kathmandu") #%I for 12 hour clock and %p for AM/PM indicator
```
...
```{r lubridate-helpers}
# the helper functions are quite flexible with input format
myd("Oct 2018, 17") # month year day
myd("Oct 2018 17")
myd("Oct-2018-17")
myd("Oct,2018,17")

# other valid helper functions are: mdy, mdy_h, mdy_hm, mdy_hms
```
...
```{r lubridate-helpers-arithmetic}
# time difference; difftime object
mdy_hm("Sep 28, 2018 12:45")-mdy_hm("Dec 30, 2016 9:12")

# add or subtract arbitrary period from a date
mdy_h("Oct 17, 2018, 12") - days(18)

# adding or subtracting is supported for a difftime object too from a date
mdy_h("Oct 17, 2018, 12") - (mdy_hm("Sep 28, 2018 12:45")-mdy_hm("Dec 30, 2016 9:12"))
```
...
```{r duration}
time_differ <- (lubridate::date() %>% 
  strptime(format = "%a %b %d %H:%M:%S %Y") %>% # suited for existing locale settings
  lubridate::as_date()) - lubridate::as_date(strptime("30-12-1992", format = "%d-%m-%Y"))

# shows duration in seconds and most readable units
time_differ %>% lubridate::as.duration()

time_differ/lubridate::dyears(1)
```
